"use strict";
module.exports =

{
    "idioma": {
        code: 402,
        es: 'Label in the header undefined, set it - ' + 'Etiqueta en la cabecera sin definir, defínala',
    },

    "usuario_01": {
        es: 'Error al guardar usuario, revise los datos',
        en: 'Error saving user, data check', code: 401
    },
    "usuario_02": {es: 'El campo nombre es obligatorio', en: 'field name required', code: 402},

    "usuario_03": {es: 'Email obligatorio', en: 'Email required', code: 402},

    "usuario_04": {es: 'Clave obligatoria', en: 'Password required', code: 402},

    "usuario_05": {
        es: 'Ya existe el usuario con esos datos',
        en: 'User already exists with that data', code: 401
    },

    "errorToken": {es: 'No se pudo autenticar el token', en: 'Failed to authenticate token', code: 402},

    "errorToken01": {es: 'token no encontrado', en: 'No token provided', code: 402},
};
